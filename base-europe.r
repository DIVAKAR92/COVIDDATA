...
source('Europe/code/utils/read-data-usa.r')
source('Europe/code/utils/process-covariates.r')
...
# Choose states
states <- as.character(unique(death_data$code))
states = c("AT", "BE", "CH", "DE", "DK", "ES", "FR", "GB", "IT", "NL", "NO", "SE")
# Read ifr 
...
# read interventions
interventions <- readRDS('Europe/data/covariates.RDS')
# read interventions lifted date
interventions_lifted <- readRDS('Europe/data/covariates_ended.RDS')
...
m <- stan_model(paste0('Europe/code/stan-models/',StanModel,'.stan'))
